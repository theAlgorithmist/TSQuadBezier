 ...
 */
 ...
 export class TSMT$TWBRF
 {
   protected static TOL: number      = 0.00001;   // hardcoded tolerance to use as default if none provided
   protected static MAX_ITER: number = 50;        // maximum iteration count
   ...
   * @default {TSMT$TWBRF.TOL}
   *
   * @returns  {number} Approximation of desired root within specified tolerance and iteration limit.  In addition to
   * too small an iteration limit or too tight a tolerance, some pathological numerical conditions exist under which
   * the method may incorrectly report a root.
   */
   public static findRoot(_x0: number, _x2: number, _f: Function, _eps: number = this.TOL)
   {
     let _imax: number = this.MAX_ITER;  // todo - allow this to be variable and user-input?
